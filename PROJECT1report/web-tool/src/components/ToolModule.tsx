...
export const ToolModule: React.FC<ToolModuleProps> = ({
  ...
  const handleRun = async () => {
    ...
    // Validation cho personal-search
    if (module.id === 'personal-search') {
      const layers = parseInt(input.layers || '0');
      const friendsArray = (input.friends_per_layer || '').split(',')
        .map(s => s.trim())
        .filter(s => s.length > 0);
      const expectedCount = layers + 1;
      
      if (friendsArray.length !== expectedCount) {
        alert(`❌ Lỗi: Cần nhập ${expectedCount} số cho ${expectedCount} tầng (0→${layers}), hiện tại có ${friendsArray.length} số.`);
        return;
      }
      
      // Kiểm tra tất cả các số đều hợp lệ
      for (let i = 0; i < friendsArray.length; i++) {
        const num = parseInt(friendsArray[i]);
        if (isNaN(num) || num < 0) {
          alert(`❌ Lỗi: Số bạn bè tầng ${i} không hợp lệ: "${friendsArray[i]}". Vui lòng nhập số nguyên dương.`);
          return;
        }
      }
    }
                ...
                <>
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-gray-300">
                      Số bạn bè mỗi tầng (cách nhau dấu phẩy)
                      {input.layers && (
                        <span className="text-blue-400 ml-1">
                          - Cần {parseInt(input.layers) + 1} số cho {parseInt(input.layers) + 1} tầng (0→{input.layers})
                        </span>
                      )}
                    </label>
                    <Input
                      placeholder={input.layers ? 
                        `VD: ${Array(parseInt(input.layers) + 1).fill('1').join(',')}` : 
                        "VD: 1,2,3"
                      }
                      ...
                      className="bg-black/30 border-white/20 text-white"
                    />
                    {input.layers && input.friends_per_layer && (
                      <div className="text-xs">
                        {(() => {
                          const expectedCount = parseInt(input.layers) + 1;
                          const actualNumbers = input.friends_per_layer.split(',')
                            .map(s => s.trim())
                            .filter(s => s.length > 0);
                          const actualCount = actualNumbers.length;
                          return actualCount === expectedCount ? (
                            <span className="text-green-400">✓ Số lượng chính xác ({actualCount} số)</span>
                          ) : (
                            <span className="text-red-400">⚠️ Cần {expectedCount} số, hiện có {actualCount} số</span>
                          );
                        })()}
                      </div>
                    )}
